...
let ProductResolver = class ProductResolver {
  ...
  products(limit, offset, type, text, category) {
    ...
    const filteredData = filter_1.filterItems(
      this.items,
      limit,
      offset,
      text,
    ...
    return new product_type_1.ProductResponse(
      Object.assign({ total: total }, filteredData)
    );
  }
  product(slug) {
  ...
  relatedProducts(slug, type) {
    return __awaiter(this, void 0, void 0, function* () {
      const relatedItem = yield filter_1.getRelatedItems(
        type,
        slug,
        this.items
      );
...
__decorate(
  [
    ...
    __param(
      0,
      type_graphql_1.Arg('limit', (type) => type_graphql_1.Int, {
        defaultValue: 10,
      })
    ),
    __param(
      1,
      type_graphql_1.Arg('offset', (type) => type_graphql_1.Int, {
        defaultValue: 0,
      })
    ),
...
__decorate(
  [
    ...
    __param(
      0,
      type_graphql_1.Arg('slug', (type) => String)
    ),
    __metadata('design:type', Function),
...
__decorate(
  [
    type_graphql_1.Query(() => [product_type_1.default], {
      description: 'Get the Related products',
    }),
    __param(
      0,
      type_graphql_1.Arg('slug', (slug) => String)
    ),
...
ProductResolver = __decorate([type_graphql_1.Resolver()], ProductResolver);
